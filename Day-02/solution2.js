...
*/
const fs = require("fs");
const { calculatePlayerRoundScore } = require("./utils");
const { Choices } = require("./constants");

const rawData = fs.readFileSync("data/input.txt").toString("utf-8");
const rounds = rawData.split("\n");

const getPlayerChoice = (opponentChoice, strategyCode) => {
  if (strategyCode === "Y") {
    return opponentChoice;
  }

  if (Choices.paper.has(opponentChoice)) {
    return strategyCode === "Z" ? "Z" : "X";
  }

  if (Choices.rock.has(opponentChoice)) {
    return strategyCode === "Z" ? "Y" : "Z";
  }

  if (Choices.scissors.has(opponentChoice)) {
    return strategyCode === "Z" ? "X" : "Y";
  }
};

let score = 0;
rounds.forEach((round) => {
  const [opponentChoice, strategyCode] = round.split(" ");
  const playerChoice = getPlayerChoice(opponentChoice, strategyCode);

  score += calculatePlayerRoundScore(playerChoice, opponentChoice);
});
