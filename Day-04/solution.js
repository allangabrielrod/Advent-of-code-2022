...
const assignmentsData = fs.readFileSync("input.txt").toString("utf-8");

const assignmentsPairs = assignmentsData.split("\n");

let totalAssignmentPairsFullyContained = 0;
let totalOverlappedAssignments = 0;

assignmentsPairs.forEach((assignmentPair) => {
  const [firstElfAssignedSections, secondElfSAssignedSections] =
    assignmentPair.split(",");
  const [firstElfStartSection, firstElfEndSection] =
    firstElfAssignedSections.split("-");
  const [secondElfStartSection, secondElfEndSection] =
    secondElfSAssignedSections.split("-");

  // Part 1
  const isFirstElfAssignmentsFullyContained =
    Number(firstElfStartSection) >= Number(secondElfStartSection) &&
    Number(firstElfEndSection) <= Number(secondElfEndSection);

  const isSecondElfAssignmentsFullyContained =
    Number(secondElfStartSection) >= Number(firstElfStartSection) &&
    Number(secondElfEndSection) <= Number(firstElfEndSection);

  if (isFirstElfAssignmentsFullyContained || isSecondElfAssignmentsFullyContained) {
    totalAssignmentPairsFullyContained++;
  }

  // Part 2
  const firstElfSection = Array.from(
    { length: Number(firstElfEndSection) - Number(firstElfStartSection) + 1 },
    (_, i) => Number(firstElfStartSection) + i
  );

  const secondElfSection = Array.from(
    { length: Number(secondElfEndSection) - Number(secondElfStartSection) + 1 },
    (_, i) => Number(secondElfStartSection) + i
  );

  const existsOverlappedAssignment = firstElfSection.find((item) =>
    secondElfSection.includes(item)
  );

  if (existsOverlappedAssignment) {
    totalOverlappedAssignments++;
  }
});
