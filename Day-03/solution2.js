...
by all three Elves. That is, if a group's badge is item type B, then all three Elves will have item type 
B somewhere in their rucksack, and at most two of the Elves will be carrying any other item type.
...
which item type is the right one is by finding the one item type that is common between all three Elves in 
each group.
...
*/
const fs = require("fs");
const { getItemPriority } = require("./utils");

const rucksacks = fs.readFileSync("input.txt").toString("utf-8");
...
for (let i = 0; i < rucksacksList.length; i += 3) {
  const groupRucksacks = rucksacksList.slice(i, i + 3);

  new Set(groupRucksacks[0]).forEach((item) => {
    const isItemInSecondRucksack = groupRucksacks[1].includes(item);
    const isItemInThirdRucksack = groupRucksacks[2].includes(item);

    if (isItemInSecondRucksack && isItemInThirdRucksack) {
      badgePriority += getItemPriority(item);
    }
  });
}
